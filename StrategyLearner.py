...
class StrategyLearner:
    ...
    def _fetch_indicators(self, symbol, sd, ed):
        ...
        compute the four technical indicators, and return:
            indicator_df  — DataFrame(SMA, BBP, MOM, PPO) indexed by date
            prices_df     — DataFrame(close) indexed by date
    ...
    def _make_labels(self, prices_df, lookahead=5):
        """
        Binary-ish classification labels from future returns:
          +1  → buy  (return > impact)
          -1  → sell (return < -impact)
           0  → hold
        """
    ...
    def add_evidence(self, symbol="SPY", sd=None, ed=None, sv=10000):
        ...
        labels = self._make_labels(prices_df, lookahead=5)

        combined = indicator_df.copy()
        combined["label"] = labels
        combined.dropna(inplace=True)

        x_train = combined[["SMA", "BBP", "MOM", "PPO"]].to_numpy()
    ...
    def testPolicy(self, symbol="SPY", sd=None, ed=None, sv=10000):
        """
        Run the trained policy over [sd, ed] and return a DataFrame of trade actions:
            +1000  buy 1000 shares
            -1000  sell 1000 shares
            +2000 / -2000  position flip
                0  no trade
        """
